"""
================================================================================
Model: Swin Transformer (Swin-T) Pretrained
================================================================================
Multi-label Thoracic Disease Classification using CXR8 Dataset

Architecture: Swin Transformer Tiny with ImageNet pretrained weights
Features:
...
class Config:
    ...
    MODEL_DIR = os.path.join(BASE_DIR, "Model_Swin")
...
def create_model(num_classes, device):
    """Create Swin-T model with pretrained weights"""
    print("\n" + "=" * 70)
    print(" MODEL ARCHITECTURE")
    print("=" * 70)
    
    print(f"\n  Loading Swin-T with ImageNet pretrained weights...")
    ...
    print(f"  Embed dim: 96")
    print(f"  Depths: [2, 2, 6, 2]")
    print(f"  Heads: [3, 6, 12, 24]")
...
def plot_training_curves(train_losses, val_losses, train_accs, val_accs, save_path):
    ...
    plt.suptitle('Swin-T Training Progress', fontsize=16, fontweight='bold', y=1.02)
    plt.tight_layout()
...
def save_description(config, save_dir):
    ...
    with open(os.path.join(save_dir, 'description.txt'), 'w') as f:
        ...
        f.write("Architecture: Swin Transformer Tiny (Swin-T)\n\n")
        
        f.write("Model Details:\n")
        f.write("-" * 40 + "\n")
        f.write("  Base: Swin-T with ImageNet pretrained weights\n")
        f.write("  Patch size: 4x4 pixels\n")
        f.write("  Window size: 7x7\n")
        f.write("  Embed dimension: 96\n")
        f.write("  Depths: [2, 2, 6, 2]\n")
        f.write("  Attention heads: [3, 6, 12, 24]\n")
        f.write("  Parameters: ~28M\n\n")
        
        f.write("Key Features:\n")
        f.write("-" * 40 + "\n")
        f.write("  - Hierarchical feature representation\n")
        f.write("  - Shifted window attention mechanism\n")
        f.write("  - Linear computational complexity\n")
        f.write("  - Better for dense prediction tasks\n\n")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("   SWIN TRANSFORMER (Swin-T) TRAINING")
    ...
    optimizer = optim.AdamW([
        {'params': model.head.parameters(), 'lr': config.LEARNING_RATE * 10},
        {'params': [p for n, p in model.named_parameters() if 'head' not in n], 'lr': config.LEARNING_RATE}
