"""
Generate all visualizations and metrics for DenseNet121 Model
...
OUTPUT_DIR = os.path.join(WORKSPACE, "Model_DenseNet121")
...
def create_model(num_classes, device):
    """Create DenseNet121 model matching training architecture"""
...
def plot_metric_comparison(metrics, output_dir):
    ...
    ax.set_title('DenseNet121 Model Performance Metrics', fontsize=14, fontweight='bold')
...
def plot_per_class_auc(y_true_bin, y_prob, classes, output_dir):
    ...
    ax.set_title('Per-Class ROC-AUC Scores (DenseNet121)', fontsize=14, fontweight='bold')
    ax.set_xlim(0.5, 1.0)
...
def plot_per_class_f1(y_true, y_pred, classes, output_dir):
    ...
    ax.set_title('Per-Class F1 Scores (DenseNet121)', fontsize=14, fontweight='bold')
...
def plot_per_class_metrics(y_true, y_pred, classes, output_dir):
    ...
    ax.set_xlabel('Class')
    ax.set_ylabel('Score')
    ax.set_title('Per-Class Precision, Recall, and F1-Score (DenseNet121)')
    ax.set_xticks(x)
...
def main():
    print("="*60)
    print("GENERATING ALL VISUALIZATIONS FOR DenseNet121 MODEL")
    print("="*60)
