...
Architecture: DenseNet121 with ImageNet pretrained weights
Features:
- Dense connections (each layer connected to every other layer)
...
Note: DenseNet121 is particularly well-suited for medical imaging tasks
and is the same architecture used in CheXNet (Stanford's chest X-ray model).
================================================================================
"""
...
class Config:
    ...
    MODEL_DIR = os.path.join(BASE_DIR, "Model_DenseNet121")
    
    # Model hyperparameters
    MODEL_NAME = "DenseNet121"
    IMAGE_SIZE = 224
    BATCH_SIZE = 32  # DenseNet is memory efficient
    EPOCHS = 10
    WARMUP_EPOCHS = 2
    LEARNING_RATE = 1e-4
    WEIGHT_DECAY = 1e-4
...
def create_model(num_classes, device):
    """Create DenseNet121 model with pretrained weights"""
    print("\n" + "=" * 70)
    print(" MODEL ARCHITECTURE")
    print("=" * 70)
    
    print(f"\n  Loading DenseNet121 with ImageNet pretrained weights...")
...
def plot_training_curves(train_losses, val_losses, train_accs, val_accs, save_path):
    ...
    plt.suptitle('DenseNet121 Training Progress', fontsize=16, fontweight='bold', y=1.02)
    plt.tight_layout()
...
def save_description(config, save_dir):
    ...
    with open(os.path.join(save_dir, 'description.txt'), 'w') as f:
        ...
        f.write("Architecture: DenseNet121 (Densely Connected Network)\n\n")
        
        f.write("Model Details:\n")
        f.write("-" * 40 + "\n")
        f.write("  Base: DenseNet121 with ImageNet pretrained weights\n")
        f.write("  Depth: 121 layers\n")
        f.write("  Dense blocks: 4\n")
        f.write("  Block configuration: [6, 12, 24, 16]\n")
        f.write("  Growth rate: 32\n")
        f.write("  Output features: 1024\n")
        f.write("  Parameters: ~8M (very efficient)\n\n")
        
        f.write("Key Features:\n")
        f.write("-" * 40 + "\n")
        f.write("  - Dense connectivity (each layer connects to all subsequent)\n")
        f.write("  - Feature reuse across layers\n")
        f.write("  - Strong gradient flow through direct connections\n")
        f.write("  - Compact model with fewer parameters\n")
        f.write("  - Used in CheXNet (Stanford) for chest X-ray analysis\n\n")
        ...
        f.write("Medical Imaging Relevance:\n")
        f.write("-" * 40 + "\n")
        f.write("  DenseNet121 is particularly suited for medical imaging because:\n")
        f.write("  - Efficient feature propagation captures subtle abnormalities\n")
        f.write("  - Dense connections help learn multi-scale features\n")
        f.write("  - Used as baseline in multiple medical imaging papers\n")
        f.write("  - CheXNet achieved radiologist-level performance with DenseNet121\n\n")
...
def main():
    ...
    optimizer = optim.AdamW([
        ...
        {'params': [p for n, p in model.named_parameters() if 'classifier' not in n], 'lr': config.LEARNING_RATE}
